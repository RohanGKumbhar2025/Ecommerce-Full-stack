...
// 🔹 IMPROVEMENT: Move these functions outside the component to prevent re-declaration on every render
...
const Navbar = ({ cartCount, wishlistCount }) => {
    ...
    // 🔹 IMPROVEMENT: Add keyboard accessibility for mobile menu
    useEffect(() => {
    ...
    // 🔹 IMPROVEMENT: Add confirmation dialog for logout
    ...
    return (
        <>
            <header className={`bg-white/95 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100 transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className={`flex justify-between items-center transition-all duration-300 ${scrolled ? 'py-2' : 'py-3'}`}>
                        ...
                        <Link to="/" onClick={handleLinkClick} aria-label="NexoShop Home">
                            <img src={logo} alt="NexoShop Logo" className="h-8 w-auto" loading="lazy" />
                            <span className="hidden sm:inline">NexoShop</span>
                        </Link>
                        
                        <DesktopNav user={user} getNavClass={getNavClass} />

                        <DesktopIcons isLoggedIn={isLoggedIn} user={user} handleLogout={handleLogout} cartCount={cartCount} wishlistCount={wishlistCount} />

                        {/* 🔹 FIX: Add focus-visible styles for accessibility */}
                        <button 
                            ref={menuButtonRef}
                            className="md:hidden p-2 text-gray-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 rounded" 
            ...
            <MobileMenu 
                ...
                getMobileNavClass={getMobileNavClass}
                location={location} // Pass the location object to MobileMenu
            />
        </>
    );
};

const DesktopNav = React.memo(({ user, getNavClass }) => (
    <nav className="hidden md:flex items-center gap-8 text-gray-600 font-medium">
        <NavLink to="/" aria-label="Home" className={getNavClass}>Home</NavLink>
        <NavLink to="/products" aria-label="Products" className={getNavClass}>Products</NavLink>
        <NavLink to="/about" aria-label="About Us" className={getNavClass}>About</NavLink>
        <NavLink to="/contact" aria-label="Contact Us" className={getNavClass}>Contact</NavLink>
        {user?.roles?.includes('ROLE_ADMIN') && (
            <NavLink to="/admin/list" aria-label="Admin Dashboard" className={getNavClass}>Admin</NavLink>
        )}
    </nav>
));

const DesktopIcons = React.memo(({ isLoggedIn, user, handleLogout, cartCount, wishlistCount }) => (
        ...
        {isLoggedIn ? (
            <>
                <Link to="/wishlist" aria-label={`View Wishlist (${wishlistCount} items)`} className="relative p-2 text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors">
                    <Heart size={22} />
                    {wishlistCount > 0 && <span aria-live="polite" className="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-semibold">{wishlistCount}</span>}
                </Link>
                <Link to="/cart" aria-label={`View Cart (${cartCount} items)`} className="relative p-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors">
                    ...
                    {cartCount > 0 && <span aria-live="polite" className="absolute top-0 right-0 bg-indigo-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-semibold">{cartCount}</span>}
                </Link>
                ...
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-sm">
                        {user?.name?.trim() ? user.name.charAt(0).toUpperCase() : <User size={20} />}
                    </div>
...
const MobileMenu = ({ isOpen, setIsOpen, isLoggedIn, user, handleLogout, handleLinkClick, cartCount, wishlistCount, getMobileNavClass, location }) => {
    return (
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        ...
                        {isLoggedIn && (
                            <div className="flex-shrink-0 p-4 bg-gray-50 border-b flex items-center gap-4">
                                <div className="w-12 h-12 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-md">
                                    {user?.name?.trim() ? user.name.charAt(0).toUpperCase() : <User size={24} />}
                                </div>
                                <div>
                        ...
                        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                            <NavLink to="/" onClick={handleLinkClick} className={getMobileNavClass} aria-current={location.pathname === '/' ? "page" : undefined}>Home</NavLink>
                            <NavLink to="/products" onClick={handleLinkClick} className={getMobileNavClass} aria-current={location.pathname.startsWith('/products') ? "page" : undefined}>Products</NavLink>
                            <NavLink to="/about" onClick={handleLinkClick} className={getMobileNavClass} aria-current={location.pathname === '/about' ? "page" : undefined}>About</NavLink>
                            <NavLink to="/contact" onClick={handleLinkClick} className={getMobileNavClass} aria-current={location.pathname === '/contact' ? "page" : undefined}>Contact</NavLink>
                            {user?.roles?.includes('ROLE_ADMIN') && <NavLink to="/admin/list" onClick={handleLinkClick} className={getMobileNavClass} aria-current={location.pathname.startsWith('/admin') ? "page" : undefined}>Admin</NavLink>}
                            ...
                            {isLoggedIn ? (
                                <>
                                    <Link to="/wishlist" onClick={handleLinkClick} className="flex items-center justify-between px-4 py-3 rounded-lg hover:bg-gray-100 font-medium text-gray-700">
                                        ...
                                        {wishlistCount > 0 && <span aria-live="polite" className="bg-red-100 text-red-700 text-sm font-semibold px-2 py-1 rounded-full">{wishlistCount}</span>}
                                    </Link>
                                    <Link to="/cart" onClick={handleLinkClick} className="flex items-center justify-between px-4 py-3 rounded-lg hover:bg-gray-100 font-medium text-gray-700">
                                        ...
                                        {cartCount > 0 && <span aria-live="polite" className="bg-indigo-100 text-indigo-700 text-sm font-semibold px-2 py-1 rounded-full">{cartCount}</span>}
                                    </Link>
